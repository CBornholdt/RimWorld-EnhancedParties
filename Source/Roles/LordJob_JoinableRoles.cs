...
{
	public enum PawnRoleLostCondition { LeftLord, InValid, NewRole, RoleDisabled };
    ...
    abstract public class LordJob_JoinableRoles : LordJob_VoluntarilyJoinable
    {
		...
		public LordJob_JoinableRoles() => Initialize();
		...
		{   //Everything is sorted descending by current role priority, allows priorities to change
            ...
            //Check for pawns that have left roles due to leaving lord or other
			foreach(var role in sortedRoles) {
				for(int i = role.currentPawns.Count - 1; i >= 0; i--) {
					...
					if(pawn.GetLord() != lord) {
                        ...
                        pawnsLostSorted.Add(Tuple.Create(role, pawn, PawnRoleLostCondition.RoleDisabled));
                        role.currentPawns.RemoveAt(i);
					}
					else
			...
			if(CurrentToil != null) {
				foreach(var pawnLost in pawnsLostSorted)
					CurrentToil.Notify_PawnLeftRole(pawnLost.Item1, pawnLost.Item2, GetRole(pawnLost.Item2));
